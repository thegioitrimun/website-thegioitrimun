...
```bash
cd "/Users/PHUC/Desktop/TGTM" && git add -A && git commit -m "deploy: cập nhật" && git push && npm run build && npx wrangler pages deploy dist --project-name=website-thegioitrimun
```
