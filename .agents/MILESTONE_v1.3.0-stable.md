# Milestone: Stable Release & Supabase Complete Recovery Guide (v1.3.0)

This document serves two purposes:
1. A guide to restore the application codebase to the stable `v1.3.0-stable` state.
...
- **Tag Name**: `v1.3.0-stable`
- **Description**: This tag marks a highly stable point featuring major feature additions and critical UI/UX fixes. Key updates include:
  - **Full i18n Strategy**: Comprehensive multi-language support (Vietnamese, English, Russian, Chinese) across static content and dynamic database records (using JSONB columns).
  - **Performance Optimizations**: Conversion of images to WebP format, implementation of lazy loading, and enhanced browser caching for significantly faster page load times.
  - **Google Avatars Integration**: Seamless handling of absolute Google Avatar URLs for both new and existing user profiles.
  - **Excel Export/Import Enhancement**: Fixed and verified the 'brand' column inclusion for product management data export/import workflows.
  - **UI/UX Polishing**: Robust cross-browser flexbox fixes (`self-stretch`, `flex-1`, `h-full`) to ensure product cards align perfectly with equal heights regardless of content length (across Homepage, Pharmacy, and Wishlist pages).
...
```bash
git checkout -b feature-from-stable v1.3.0-stable
```
...
1. Go to **Authentication > Configuration > Providers** and enable your desired providers (e.g., Email, Google).
